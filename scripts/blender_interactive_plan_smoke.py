...
OUTPUT_PATH = Path("/tmp/bonsai_ai_interactive_plan_smoke.json")
...
def _run():
    ...
    with bpy.context.temp_override(**context_data):
        try:
            ...
            payload["plan_length"] = len(bpy.data.texts[bpy.context.scene.bonsai_ai_settings.plan_text_name].as_string())
